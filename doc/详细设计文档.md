...
| HotelController.addRoomInfo                    | public ResponseVO addRoomInfo(@RequestBody HotelRoom hotelRoom) | 已创建一个RoomSerivce对象，且输入符合输入规则  | 调用RoomSerivce的insertRoomInfo方法                  |
...
| HotelController.updateHotelInfo                | ublic ResponseVO updateHotelInfo(@RequestBody UpdateHotelVO updateHotelVO) | 已创建一个HotelService对象，且输入符合输入规则 | 调用HotelService的updateHotelInfo方法                |
...
| 提供的服务（接口）                        | 语法                                                         | 前置条件                                       | 后置条件                                        |
| ----------------------------------------- | ------------------------------------------------------------ | ---------------------------------------------- | ----------------------------------------------- |
| OrderController.reserveHotel              | public ResponseVO reserveHotel(@RequestBody OrderVO orderVO) | 已创建一个OrderService对象，且输入符合输入规则 | 调用OrderService的addOrder方法                  |
...
| OrderController.checkOut                  | public ResponseVO checkOut(@PathVariable int orderId)        | 已创建一个OrderService对象，且输入符合输入规则 | 调用OrderService的checkOutOrder方法             |
